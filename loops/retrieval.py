"""Module for computing retrieval metrics."""
import numpy as np
import scipy.stats
...
def t2v_metrics(sims, query_masks=None):
    ...
    if cols.size != num_queries:
        raise ValueError(msg.format(cols.size, num_queries))
    ...
    return cols2metrics(cols, num_queries), cols
...
def v2t_metrics(sims, query_masks=None):
    ...
    MISSING_VAL = 1E8  # pylint: disable=invalid-name
    query_ranks = []
    for ii in range(num_queries):  # pylint: disable=invalid-name
        row_dists = dists[ii, :]
        if query_masks is not None:
        ...
        min_rank = np.inf
        for jj in range(ii * caps_per_video, (ii + 1) * caps_per_video):  # pylint: disable=invalid-name
            ...
            if break_ties == "optimistically":
                ...
                raise ValueError(f"unknown break_ties: {break_ties}")
            if rank < min_rank:
                min_rank = rank
        query_ranks.append(min_rank)
    query_ranks = np.array(query_ranks)

    return cols2metrics(query_ranks, num_queries), query_ranks


def cols2metrics(cols, num_queries):
    """
    Compute retrieval metrics from a column vector of ranks.
    """
    metrics = {}
...
if __name__ == "__main__":
    # test out the implementation with a toy example
    test_sims = np.ones((3, 3))
    test_sims[0, 0] = 2
    test_sims[1, 1:2] = 2
    test_sims[2, :] = 2
    test_query_masks = None  # pylint: disable=invalid-name

    print("v2t_metrics")
    print(v2t_metrics(test_sims, test_query_masks))

    print("t2v_metrics")
    print(t2v_metrics(test_sims, test_query_masks))
